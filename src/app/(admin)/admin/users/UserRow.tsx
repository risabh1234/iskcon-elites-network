...
export function UserRow({ user, isSelf }: { user: UserDto; isSelf: boolean }) {
  ...
  return (
    <TableRow>
      ...
      <TableCell>
        {/* Disabled for your own row: changing your own role is the one edit
            that can silently lock the institution out of its own console. The
            policy refuses it too — this only saves the round trip. */}
        {isSelf ? (
